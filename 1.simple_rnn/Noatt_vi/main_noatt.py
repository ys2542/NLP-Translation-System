...
from att_model import EncoderRNN, AttnDecoderRNN, DecoderRNN
from train_noatt import masked_cross_entropy, train, evaluate, as_minutes, time_since
...
decoder = DecoderRNN(hidden_size, lang_en_vi.n_words, n_layers, dropout=dropout).to(device)
...
while epoch < n_epochs:
    ...
    torch.save(encoder.state_dict(), "noatt_encoder_vi")
    torch.save(decoder.state_dict(), "noatt_decoder_vi")
        
out = {"train_loss_1000_step": train_loss_step, 
       "validation_loss_1000_step": val_loss_step
       }

with open('noatt_output_vi.txt', 'w') as file:
